...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <xsl:template match="/">
    <html>
      <head>
        <title>ProUnit Execution and Database Activity Report</title>
        <link href="styles_prounitdbmonitor.css" rel="stylesheet" type="text/css" />
  ...
  <xsl:template match="Test" mode="results">
    ...
    <xsl:variable name="currentId" select="../@seq" />
    <xsl:variable name="currentTest" select="@name" />
    <xsl:variable name="activityCounter" select="count(//ProUnitExecutionLog/DBMonitor/TestCase[@id=$currentId]/TestName[@name=$currentTest]/DBStat)" />
    <xsl:if test="$activityCounter > 0">
      <tr>
        <td colspan="5" align="center">
          <table border="0" cellspacing="0" style="border-collapse: collapse;">
            <tr class="ResultHeader" align="center"><td colspan="5" class="ResultCell">Database Activity</td></tr>
            <tr class="ResultHeader">
              <td class="ResultCell" align="center">Table</td>
              <td class="ResultCell" align="center">Reads</td>
              <td class="ResultCell" align="center">Updates</td>
              <td class="ResultCell" align="center">Creates</td>
              <td class="ResultCell" align="center">Deletes</td>
            </tr>
            <xsl:for-each select="//ProUnitExecutionLog/DBMonitor/TestCase[@id=$currentId]/TestName[@name=$currentTest]/DBStat">
              <tr>
                <td class="ResultCell ReportValue" align="center"><xsl:value-of select="@table" /></td>
                <td class="ResultCell ReportValue" align="center"><xsl:value-of select="@reads" /></td>
                <td class="ResultCell ReportValue" align="center"><xsl:value-of select="@updates" /></td>
                <td class="ResultCell ReportValue" align="center"><xsl:value-of select="@creates" /></td>
                <td class="ResultCell ReportValue" align="center"><xsl:value-of select="@deletes" /></td>
              </tr>
            </xsl:for-each>
          </table>
        </td>
